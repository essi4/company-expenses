...
export default function BeautyWorkspace({ name, mode, plan, businessSlug }: { name: string; mode: string; plan: string; businessSlug?: string }) {
  ...
  async function addPayment() {
    ...
    if (!resolvedBusinessId) return;
    const customer = customers.find((x) => x.name === paymentForm.customer);
    const service = services.find((x) => x.name === paymentForm.service);
    if (!customer || !service) return notify("اطلاعات پرداخت ناقص است.");
    const method = paymentForm.method === "کارت" ? "card_terminal" : paymentForm.method === "انتقال" ? "transfer" : "cash";
    const { data, error } = await supabase.from("business_payments").insert({
      business_id: resolvedBusinessId, customer_id: customer.id, service_id: service.id, amount, method, status: "paid"
    }).select("id,amount,method").single();
    if (error || !data) return notify("ثبت پرداخت انجام نشد.");
    const label = data.method === "card_terminal" ? "کارت" : data.method === "transfer" ? "انتقال" : "نقدی";
    setPayments((items) => [...items, { id: data.id, customer: customer.name, service: service.name, amount: Number(data.amount), method: label }]);
  ...
  async function createInvoice() {
    ...
    const { data: generatedNumber, error: numberError } = await supabase.rpc("next_business_document_number", { p_business_id: resolvedBusinessId, p_document_type: "invoice" });
    if (numberError || !generatedNumber) return notify("شماره فاکتور ایجاد نشد.");
    const invoiceNumber = `${name.slice(0, 2).toUpperCase()}-${generatedNumber}`;
