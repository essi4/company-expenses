              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-4 sm:p-5">
                <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_170px_170px_auto]">
                  ...
                  <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-sm font-bold text-slate-300">
                    <option value="all">همه مدل‌ها</option><option value="Beauty">زیبایی</option><option value="Automotive">خودرو</option><option value="Medical">پزشکی</option><option value="Retail">خرده‌فروشی</option><option value="Services">خدمات</option><option value="Hospitality">مهمان‌نوازی</option><option value="Education">آموزش</option><option value="Fitness">ورزش</option><option value="Professional">حرفه‌ای</option><option value="Other">سفارشی</option>
                  </select>
                    ...
                    <tbody className="divide-y divide-white/10">
                      {filtered.map((business) => (
                        <tr key={business.id} className="hover:bg-white/[0.025]">
                          ...
                          <td className="px-4 py-4"><span className="block text-xs font-bold text-slate-200">{getBusinessBlueprint(business.type).title}</span><span className="text-[10px] text-slate-500">{modeLabel(business)}</span></td>
